                ...
                for metric_name, metric_val in metric2val.items():
                    if (var_name == core_var) and any(metric_name.startswith(pfx) for pfx in ["mean", "maj", "best"]) and (f"@{n_max}" in metric_name):
                        ...
                        if metric_name.startswith("mean") and (f"@{n_max}" in metric_name):
                            avg_rate.append(metric_val)
                    else:
                        metric_sec = "val-aux"
                    pfx = f"{metric_sec}/{data_source}/{var_name}/{metric_name}"
                    metric_dict[pfx] = metric_val
        
        # compute overall avg@16
        pfx = f"val-core/overall"
        metric_dict[pfx] = np.mean(avg_rate)
                ...
                with _timer("step", timing_raw):
                    ...
                    with _timer('reward', timing_raw):
                        ...
                        try:
                            ...
                            delta_reward_tensor = reward_result['delta_reward_tensor']
                            reward_extra_infos_dict = reward_result['reward_extra_info']
                        except Exception as e:
                            print(f'Error in reward_fn: {e}')
                            reward_tensor, delta_reward_tensor = self.reward_fn(batch)
                            reward_extra_infos_dict = {}

                        batch.batch['token_level_scores'] = reward_tensor
                        batch.batch['delta_score'] = delta_reward_tensor
                    ...
                    with _timer("old_log_prob", timing_raw):
                        ...
                        if 'entropys' in batch.batch:
                            metrics.update(
                                {
                                    "actor/entropy": torch.mean(batch.batch['entropys'].detach()).item(),
                                }
                            )
                        if 'full_entropys' in batch.batch:
                            metrics.update(
                                {
                                    "actor/full_entropy": torch.mean(batch.batch['full_entropys'].detach()).item(),
                                }
                            )
                    ...
                    with _timer("adv", timing_raw):
                        ...
                        prompt_uid2metric_accs = defaultdict(list)
                        prompt_uid2metric_lens = defaultdict(list)

                        for idx, uid in enumerate(batch.non_tensor_batch['uid']):
                            prompt_uid2metric_accs[uid].append(batch.non_tensor_batch['acc'][idx])
                            prompt_uid2metric_lens[uid].append(batch.non_tensor_batch['length'][idx])
                        ...
                        for uid, accs in prompt_uid2metric_accs.items():
                            acc = np.mean(accs)
                            avg_lens = np.mean(prompt_uid2metric_lens[uid])

                            if acc == 1:
                                all_correct_length.append(avg_lens)
                            elif acc == 0: 
                                all_wrong_length.append(avg_lens)
                            else:
                                other_length.append(avg_lens)

                            if acc > 0.8:
                                easy_length.append(avg_lens)

                            elif acc < 0.2:
                                hard_length.append(avg_lens)
                            else:
                                medium_length.append(avg_lens)
                            

                        metrics['batch/all_correct_count'] = len(all_correct_length)
                        metrics['batch/all_correct_length'] = np.mean(all_correct_length) if len(all_correct_length) > 0 else 0
                        
                        metrics['batch/all_wrong_count'] = len(all_wrong_length)
                        metrics['batch/all_wrong_length'] = np.mean(all_wrong_length) if len(all_wrong_length) > 0 else 0

                        metrics['batch/other_count'] = len(other_length)
                        metrics['batch/other_length'] = np.mean(other_length) if len(other_length) > 0 else 0

                        metrics['batch/easy_count'] = len(easy_length)
                        metrics['batch/easy_length'] = np.mean(easy_length) if len(easy_length) > 0 else 0

                        metrics['batch/hard_count'] = len(hard_length)
                        metrics['batch/hard_length'] = np.mean(hard_length) if len(hard_length) > 0 else 0

                        metrics['batch/medium_count'] = len(medium_length)  
                        metrics['batch/medium_length'] = np.mean(medium_length) if len(medium_length) > 0 else 0    
                    ...
                    if self.config.trainer.critic_warmup <= self.global_steps:
                        ...
                        # self.entropy_anchor = metrics['actor/entropy_anchor']
