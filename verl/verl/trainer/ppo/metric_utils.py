...
def compute_data_metrics(batch: DataProto, use_critic: bool = True) -> Dict[str, Any]:
    ...
    delta_score = batch.batch["delta_score"].sum(-1)
    ...
    metrics = {
        ...
        # delta score
        "critic/delta_score/mean": torch.mean(delta_score).detach().item(),
        "critic/delta_score/max": torch.max(delta_score).detach().item(),
        "critic/delta_score/min": torch.min(delta_score).detach().item(),
        # reward
        ...
        **(
        ...
        "reward/score/mean": np.mean(batch.non_tensor_batch['score']),
        "reward/score/max": np.max(batch.non_tensor_batch['score']),
        "reward/score/min": np.min(batch.non_tensor_batch['score']),
        "reward/acc/mean": np.mean(batch.non_tensor_batch['acc']),
        "reward/acc/max": np.max(batch.non_tensor_batch['acc']),
        "reward/acc/min": np.min(batch.non_tensor_batch['acc']),
