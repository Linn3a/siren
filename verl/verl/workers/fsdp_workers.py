...
class ActorRolloutRefWorker(Worker):
    ...
    def clean_target_entropy(self):
        self.actor.clean_target_entropy()
    ...
    def update_actor(self, data: DataProto):
        ...
        if dist.is_initialized():
            ...
            output = DataProto(meta_info={"metrics": aggregated_metrics, "all_metrics": all_metrics})
    ...
    def generate_sequences(self, prompts: DataProto):
        ...
        meta_info = {
            ...
            "n": prompts.meta_info["n"] if "n" in prompts.meta_info else self.config.rollout.n,
            "response_length": prompts.meta_info["response_length"] if "response_length" in prompts.meta_info else self.config.rollout.response_length,
        }
    ...
    def compute_log_prob(self, data: DataProto):
        ...
        with self.ulysses_sharding_manager:
            ...
            output, entropys, full_entropys = self.actor.compute_log_prob(data=data, calculate_entropy=True)
            if entropys is not None and full_entropys is not None:
                output = DataProto.from_dict(
                    tensors={"old_log_probs": output, "entropys": entropys, "full_entropys": full_entropys},
                    meta_info={"temperature": self.config.rollout.temperature},
                )
            elif entropys is not None:
                output = DataProto.from_dict(
    ...
    def save_checkpoint(self, local_path, hdfs_path=None, global_step=0, max_ckpt_to_keep=None):
        ...
        if self.rank == 0:
            path = os.path.join(os.path.dirname(local_path), "hf")
            print(f'Saving actor checkpoint to {path}')
            os.makedirs(path, exist_ok=True)
            self.actor_module.save_pretrained(path, state_dict=state_dict)
...
class RewardModelWorker(Worker):
    ...
    def __init__(self, config):
        ...
        self.model_type = self.config.model.get("model_type", "token_classification")
    ...
    def _forward_micro_batch(self, micro_batch):
        ...
        with torch.no_grad(), torch.autocast(device_type="cuda", dtype=torch.bfloat16), torch.distributed.fsdp.FullyShardedDataParallel.summon_full_params(self.reward_module):
            ...
            if self.model_type == "token_classification":
                ...
                import re
                output = self.reward_module.generate(
                    input_ids=input_ids,
                ...
                # 提取生成内容
                generated = output[:, input_ids.shape[-1]:]  # only new tokens
                ...
                for text in texts:
                    try:
                        number = float(re.findall(r"[-+]?\d*\.\d+|\d+", text)[0])
                    except:
                        number = 0.0
    ...
    def _add_reward_model_prompt(self, data: DataProto):
        src_max_length = data.batch["attention_mask"].shape[-1]
        ...
        reward_system_prompt = [{
            "role": "system",
            "content": """Fast thinking refers to responses that are quick, intuitive, and concise, with minimal reasoning or elaboration. In contrast, slow thinking is more deliberate, structured, and involves extended reasoning.

Your task is to score the input answer on a continuous scale from 0.00 to 1.00, according to the following guidelines:
- Score 1.00 if the answer is clearly fast, intuitive, and concise, with little or no reasoning.
- Score 0.00 if the answer is clearly slow, deliberate, and reasoned in depth.
- Assign intermediate scores (e.g., 0.25, 0.50, 0.75, etc.) if the answer reflects a mix of both fast and slow thinking. The more it leans toward fast, intuitive responses, the closer to 1.00; the more it leans toward slow, reasoned thinking, the closer to 0.00.

Additional instructions:
- Do not evaluate factual correctness, tone, or writing quality.
- Focus only on the nature of the thought process (i.e., speed vs. deliberation).
- Output only a single number between 0.00 and 1.00, rounded to two decimal places.
- The higher the score, the more the answer reflects fast, intuitive thinking."""
        }]
        for i in range(data.batch.batch_size[0]):
            # extract raw prompt
            if isinstance(data.non_tensor_batch["raw_prompt"][i], list):
    ...
    def compute_rm_score(self, data: DataProto):
        ...
        data = data.to(torch.cuda.current_device())
        if self.model_type != "token_classification":
            rm_data = self._add_reward_model_prompt(data)   
